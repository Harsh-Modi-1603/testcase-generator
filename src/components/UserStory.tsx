...
import { FileText, Tag, Clock, Calendar, User, CheckSquare, PenLine } from "lucide-react";
      ...
      <CardHeader className="pt-5 pb-3 flex flex-row items-start gap-4">
        <div className="flex-1">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <Badge className={cn("story-tag", statusColorMap[status])}>
              {status}
            </Badge>
            <Badge className={cn("story-tag", priorityColorMap[priority])}>
              {priority}
            </Badge>
          </div>
          <h3 className="text-lg font-medium leading-6 text-balance">{title}</h3>
        </div>
      </CardHeader>
        ...
        >
          {isGenerating ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <PenLine size={16} className="ml-auto transition-transform group-hover:translate-x-1" />
            </div>
          )}
        </Button>
      </CardFooter>
